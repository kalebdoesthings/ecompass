...
.catch(err => showNotification("Error", "Something went wrong", false));
...
function goneTime(timeLeft) {
            ...
            let timeGone = `${goneHours}:${goneMinutes}:${goneSeconds}`
...
fetch('/active_pass/' + studentId)
    ...
    .then(data => {
        if (data.status === "ok") {
            ...
            console.log(data)
            var locFrom = capitalizeFirstLetter(data.pass[3]);
            var locTo = capitalizeFirstLetter(data.pass[4]);
    ...
    .catch(err => console.error("Failed to load active pass:", err));
...
// shows or hides custom location textareas when other is picked
document.addEventListener("DOMContentLoaded", function() {
...
document.querySelector('form').addEventListener('submit', function(e) {
  ...
  // if other is picked use the textarea value instead
 ...
 .then(res => {
  ...
  .then((data) => {
    ...
    // swap the from with the to and get rid of the to
